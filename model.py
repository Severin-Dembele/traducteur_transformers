...
class PositionalEncoding(nn.Module):
    ...
    def  __init__(self, d_model:int, seq_len: int, dropout:float=float) -> None:
        super().__init__()
        ...
        #Apply the sin to even positions  and   values  of the  model   to odd  positions
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = attention_scores.softmax(dim=-1) # softmax 
...
class Encoderblock(nn.Module):
    ...
    def forward(self, x, src_mask):
        x = self.residual_connections[0](x,lambda x: self.feed_forward_block(x,x,x,src_mask))
